...
let grid=[];
const SOLID=new Set(['T','W','H','N','M','S','B','R','V','C','U','r','b','K','w','Y','X']);
...
function bakeMisc(kind){
  ...
  if(kind==='firebolt'){const cv=mkCv(36,16);ctx.translate(18,8);
    ctx.shadowColor='#ff9a3c';ctx.shadowBlur=6;
    ...
    ctx.shadowBlur=0;
    ctx.fillStyle='#ffc23c';ctx.beginPath();
    ...
    ctx.fillStyle='#fff6d8';ctx.beginPath();ctx.ellipse(7,0,4.5,1.9,0,0,7);ctx.fill();return cv;}
...
const SPELLBOOK={
  ...
  rune:{name:()=>{const n=SKILLS.find(s2=>s2.key===runeKey());return n?n.name:'Rune';},
...
function openCharacter(sel){if(typeof sel!=='string')sel=null;const p=document.getElementById('panel');const M=mods();
  ...
  let h=`<h2>${nm} — Lv.${G.level}${fullSet()?' <span style="color:#2fd06a">🌟</span>':''}</h2>`;
...
function openSkills(sel){if(typeof sel!=='string')sel=null;const p=document.getElementById('panel');
  ...
  if(nd){const rank=(G.skills||{})[nd.key]||0,maxed=rank>=nd.max,ok=nodeOpen(nd);
    ...
    h+=`<div class="pcard" style="max-width:500px;margin:4px auto 0;width:100%">
      <div style="flex:1;min-width:0"><b>${nd.name}</b> <span class="lv">${rank}/${nd.max}</span><br>
...
function applyGearFx(){if(!scene||!scene.pGlow)return;
  ...
  if(fullSet()){scene.pGlow.setTint(0x9aff6a).setScale(1.0).setAlpha(0.5);
    ...
    scene.pGlow.setTint(wC?Phaser.Display.Color.HexStringToColor(wC).color:0xffe9a0);}
...
class World extends Phaser.Scene{
  ...
  unleashSuper(){
    ...
    if(cls==='warrior'){
      ...
      for(let i=0;i<14;i++){const a=i/14*6.283;this.spawnShot(a,480,Math.max(6,Math.floor(base*1.6)),2,0xff8844);}
  ...
  armRune(){
    ...
    if(this.runeArm)toast({mage:'🔥 Swipe where the Fire Wall should go!',warrior:'⛰ Swipe to shatter the earth!',ranger:'🌿 Swipe to grow the Thorn Wall!'}[cls]);}
  castRune(pts){
    this.runeArm=false;
    ...
    toast({mage:'🔥 FIRE WALL!',warrior:'⛰ EARTHSHATTER!',ranger:'🌿 THORN WALL!'}[cls]);}
  updateZones(dt){
  ...
  updateWaves(dt){
    ...
    for(let i=this.waves.length-1;i>=0;i--){const w=this.waves[i];w.r+=430*dt;
      ...
      this.waveGfx.lineStyle(8*k+2,cls==='warrior'?0xffb454:cls==='ranger'?0xa8e063:0xff8844,Math.max(0,k*0.8));
  ...
  attack(spell,gx,gy,forceAng){
    ...
    if(cls==='warrior'){
      ...
      for(let i=0;i<n;i++){const a2=ang+i*(Math.PI*2/n);
        this.spawnShot(a2,430,dmg,(cls==='ranger'?1:0)+M.pierce,cls==='mage'?0x7ad0ff:0xa8e063,cls==='ranger');}
      ...
      for(let i=0;i<n;i++){const a2=ang+(i-(n-1)/2)*0.28;
        this.spawnShot(a2,cls==='ranger'?600:480,dmg,(spell==='chain'?2:(cls==='ranger'?1:0))+M.pierce,cls==='mage'?0xff8844:0xbbaaff);
      }
    }
  }
  hurtEnemy(e,dmg,ang){
    ...
    if(e.hp<=0){
      ...
      grantXp((6+e.lvl*3)*(e.alpha?3:e.rare?2:1));sfx('coin');
      if(e.alpha){this.dropGear(e.x,e.y,rollGear(e.lvl,2));toast('⭐ Defeated the ALPHA '+e.sp+'!');}
  ...
  update(_,dms){
    ...
    this.updateWaves(dt);this.updateZones(dt);if(!dungeon){this.updateTowers(dt);this.updateTraps(dt);this.updateRaid(dt);}
    ...
    for(let i=this.enemies.length-1;i>=0;i--){const e=this.enemies[i];
      ...
      if(!dungeon&&!e.raider&&dl>TILE*24){e.s.destroy();e.hbB.destroy();e.hbF.destroy();if(e.nameT)e.nameT.destroy();if(e.crown)e.crown.destroy();if(e.ring)e.ring.destroy();this.enemies.splice(i,1);this.spawnEnemy();continue;}
    ...
    for(let i=this.shots.length-1;i>=0;i--){const s=this.shots[i];
      ...
      if(s.core.texture.key==='arrowTex'||s.core.texture.key==='fireboltTex'){
...
document.querySelectorAll('.classbtn').forEach(b=>b.onclick=()=>{
  cls=b.dataset.cls;newGame(cls);boot();});
if(load()){
  const row=document.querySelector('#ovl div');
  const cont=document.createElement('button');cont.className='classbtn';cont.textContent='▶ Continue Lv.'+G.level+' '+G.class;
  cont.style.background='linear-gradient(#ffd84a,#f0a012)';
  cont.onclick=()=>{cls=G.class;boot();};
  row.parentNode.insertBefore(cont,row);
}
